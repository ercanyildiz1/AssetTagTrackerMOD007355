...
<html> <!--html Tag is a container used to contain all the HTML Code together-->
	<head> <!--head Tag is a container used to contain everything below in the header-->
		<title>ITSERVICES.GROUP/ASSETTRACKER | Search An Asset</title> <!--title Tag is used to give the website a name on the tab of the browser--> 
	...
	<body> <!--'body' Tag is a container used to contain the main part of the website which users interact with-->
		...
		<center> <!--'center' Tag is a container which centralises all displayed text within tbe container-->
			...
			<h2>Search An Asset</h2><!--'h2'Tag is a used to display text in large format (in this case a header) and is styled in the style sheet with sizing, font and colour--> 
			<br><!--the 'br' tag is used to 'break' (space out) whats printed on the website -->
			<p><a href="Home_Page.php">Home</a>  |  <a href="Asset_Search.php"> Refresh</a></p>
            <form action="Asset_Search.php" method="post"> <!--starts the form and gives provides the method of which the form will be handles-->
            ...
            <?php
                ...
                if (isset($_POST['search'])) ///This if statement is checking if submit has been pressed if so it will carry out the code below///
                    {
                        $query = $_POST ['query']; ///Variable '$query' is being assigned the value 'query' through the global variable '$_REQUEST' ///
                        $MYSQL = "SELECT * FROM `Assets_Table` WHERE `Device_Asset_Tag_ID` LIKE '%$query%' OR `Device_Type` LIKE '%$query%' OR `Device_Brand` LIKE '%$query%' OR `Device_Model` LIKE '%$query%' OR `Device_Serial_Number` LIKE '%$query%' OR `Device_IMEI_Number` LIKE '%$query%' OR `Device_Assigned_To_First_Name` LIKE '%$query%' OR `Device_Assigned_To_Last_Name` LIKE '%$query%' OR `Device_Assigned_To_Email_Address` LIKE '%$query%' OR `Device_Assigned_To_Employee_Number` LIKE '%$query%' OR 'Device_Time_Stamp' LIKE '%$query%' OR 'Device_Adjustment_Timestamp' LIKE '%$query%'"; ///prepared MYSQL statement to query everything from the table///
