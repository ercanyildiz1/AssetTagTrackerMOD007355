...
<html> <!--html Tag is a container used to contain all the HTML Code together-->
	...
	<body> <!--'body' Tag is a container used to contain the main part of the website which users interact with-->
		...
		<center> <!--'center' Tag is a container which centralises all displayed text within tbe container-->
			...
			<p><a href="Home_Page.php">Home</a>  |  <a href="UserAuth_Update.php"> Refresh</a></p>
			...
			<?php
				...
				echo "<table border='1'>";
					...
					$OUTPUT = mysqli_query($MYSQLLINK,"SELECT * FROM UserAuth_Table"); ///'$OUTPUT' varibale is assigned a mysql query to select everything from the 'Assets_Table'///
					...
					while($row = mysqli_fetch_array($OUTPUT)) ///The while loop is used to assign the variable '$row" with the values output by 'mysqli_fetch_array' in '$OUTPUT'///
						{
							echo "<tr>"; ///Opening of container 'tr' for the data being output from the database///
								...
								echo '<td><a href="UserAuth_Update_Form.php?action=update&UserAuth_ID='.$row['UserAuth_ID'].'">Update</a></td>'; /// Creates link button to execute the update///
								echo '<td><a href="UserAuth_Delete_Form.php?action=delete&UserAuth_ID='.$row['UserAuth_ID'].'">Delete</a></td>'; /// Creates link button to execute delete///
